...
func newVolume(path string, mask UsnReason) (*volume, error) {
...
func openVolume(path string) (syscall.Handle, error) {
	...
	supported, err := IsSupportedUsnJournal(hVolume)
	if err != nil {
		syscall.CloseHandle(hVolume)
		return syscall.InvalidHandle, errors.Wrap(err)
	}
...
func getVolumePathName(path string) (string, error) {
	...
	err = GetVolumePathName(pathUTF16, &bufferPathName[0], uint32(lengthPathName))
	if err != nil {
		return "", errors.Wrap(err)
	}
...
func getVolumeName(path string) (string, error) {
	...
	err = GetVolumeNameForVolumeMountPoint(bufferPathName, &bufferName[0], uint32(lengthName))
	if err != nil {
		return "", errors.Wrap(err)
	}
